...
echo 'Environment="KUBELET_EXTRA_ARGS=--node-ip=10.0.0.#{i + 10}"' | sudo tee -a /etc/systemd/system/kubelet.service.d/10-kubeadm.conf
...
for (( i=1; i <= #{NODE_COUNT}; i++ )); do
  addr=$((10 + $i))
  echo "10.0.0.$addr node$i" >> /etc/hosts
done
